% Modified Monte Carlo risk-neutral pricing of down-and-out put option 
% v = mod_mcdao(Price, Strike, Rate, Time, Maturity, Volatility, ...
...
function v = mod_mcdao(Price, Strike, Rate, Time, Maturity, Volatility, ...
    Barrier, Rebate, Time_step, Paths)
...
for i = 1:m
    ...
    v(i, 1) = payoff(sPath, k, b, fZero, sigma, delta, n);
end
v = exp(-r) * sum(v)/m;
end

function v = payoff(s, k, b, fZero, sigma, delta, n)
if any(s - b <= 0)
    v = fZero;
else
    pVect = exp(-2 * (b^2 + s(1:end-1).*s(2:end) - (s(1:end-1) + ...
        s(2:end)) * b) ./ (sigma * s(1:end-1).^2 * delta));
    if any(pVect >= rand(1, n))
        v = fZero;
    else
        v = max(k - s(end), 0);
    end
